...
puts "# ==================================================================================================== #"
puts "This is an OpenSees model for the 4-span San Jose bridge model (55-0909G):"
puts "  1. Single element for each span"
...
# ======================================================================
set NPiers 3
set Lpier2  [expr 25.810*$ft]
set Lpier3  [expr 24.636*$ft]
set Lpier4  [expr 22.165*$ft]

set X_PIER2 [expr 127.953*$ft]
set X_PIER3 [expr 301.837*$ft]
set X_PIER4 [expr 505.249*$ft]
set X_ABUT2 [expr 623.360*$ft]

set ZrigidLink [expr 10.942*$ft] 

# ======================================================================
# NODE DEFINITION AND RESTRAINT
# ======================================================================
# Abutments 
node 101 0.0      $Lpier2 0.0;  # ABUTMENT 1
node 105 $X_ABUT2 $Lpier4 0.0;  # ABUTMENT 2

# Piers base 
node 2 $X_PIER2 0.0  0.0;       # PIER BASE 
node 3 $X_PIER3 0.0  0.0;       # PIER BASE 
node 4 $X_PIER4 0.0  0.0;       # PIER BASE 

# Piers top 
node 102 $X_PIER2 $Lpier2 0.0;  # PIER TOP
node 103 $X_PIER3 $Lpier3 0.0;  # PIER TOP
node 104 $X_PIER4 $Lpier4 0.0;  # PIER TOP

# Rigid Beam nodes
node 211 0.0 $Lpier2  $ZrigidLink  
node 212 0.0 $Lpier2 -$ZrigidLink 
node 411 0.0 $Lpier2  $ZrigidLink     
node 412 0.0 $Lpier2 -$ZrigidLink  

node 221 $X_ABUT2  $Lpier4  $ZrigidLink  
...
if {$SectionType == "Elastic"} {
   element elasticBeamColumn $col2 4 3 $Ap $Ec $Gc $J $Ipy $Ipz $PierTransfTag;
...
source build.RCSection.Circ3D.tcl; # or source build.HollowSection3D.tcl;
   ...
   BuildRCcircSection $P2_ColTag $IDconcCore $IDconcCover $IDSteel $P2_Dsec $P2_cover $P2_barNum $P2_barArea $PierSecTagTorsion $P2SecTag3D
   BuildRCcircSection $P3_ColTag $IDconcCore $IDconcCover $IDSteel $P3_Dsec $P3_cover $P3_barNum $P3_barArea $PierSecTagTorsion $P3SecTag3D
   BuildRCcircSection $P4_ColTag $IDconcCore $IDconcCover $IDSteel $P4_Dsec $P4_cover $P4_barNum $P4_barArea $PierSecTagTorsion $P4SecTag3D
   ...
   # nonlinear piers
   element beamWithHinges $col2 2 102 $P2_ColTag $Lpi $P2_ColTag $Lpj $Ec $Ap [expr 0.7*$Ipz] [expr 0.7*$Ipy] $Gc $J $PierTransfTag
   element beamWithHinges $col3 3 103 $P3_ColTag $Lpi $P3_ColTag $Lpj $Ec $Ap [expr 0.7*$Ipz] [expr 0.7*$Ipy] $Gc $J $PierTransfTag
   element beamWithHinges $col4 4 104 $P4_ColTag $Lpi $P4_ColTag $Lpj $Ec $Ap [expr 0.7*$Ipz] [expr 0.7*$Ipy] $Gc $J $PierTransfTag
   puts "NL Beam with Hinges $col2 from 2 to 102"
   puts "NL Beam with Hinges $col2 from 2 to 103"
...
element elasticBeamColumn $gird1 101 301 $Ad $Ec $Gc $J [expr $Idy] [expr $Idz] $GirderTransfTag -mass $eleMass
element elasticBeamColumn $gird2 302 303 $Ad $Ec $Gc $J [expr $Idy] [expr $Idz] $GirderTransfTag -mass $eleMass
element elasticBeamColumn $gird3 304 305 $Ad $Ec $Gc $J [expr $Idy] [expr $Idz] $GirderTransfTag -mass $eleMass
element elasticBeamColumn $gird4 306 105 $Ad $Ec $Gc $J [expr $Idy] [expr $Idz] $GirderTransfTag -mass $eleMass
...
mass 102 [expr $F2/$g] 0. [expr $F2/$g] 0. 0. 0.;       # PIER TOP
mass 103 [expr $F3/$g] 0. [expr $F3/$g] 0. 0. 0.;       # PIER TOP
mass 104 [expr $F4/$g] 0. [expr $F4/$g] 0. 0. 0.;       # PIER TOP
...
pattern Plain $PatternTag Linear {
   ...
   eleLoad -ele 101 102 103 104 -type -beamUniform 0.0 -0.925
}
